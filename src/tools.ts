...
import type { MemoryEntry, MemoryStore } from './store.ts'
...
export function registerMemoryTools(
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      ...
      if (content.length === 0) throw new Error('memory_write: content must be non-empty')
      if (content.length > 2000) throw new Error('memory_write: content exceeds 2000 characters')
    ...
    presentCall: args => ({ card: 'generic' as const, title: 'Write memory', rawInput: `${args.kind}: ${args.content}` }),
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      ...
      if (query.length === 0) throw new Error('memory_search: query must be non-empty')
      const caller = callerOf(exec)
      ...
      const hits = await services.store.search(vector, {
        kinds: args.kind === undefined ? undefined : [args.kind as MemoryKind],
        tags: normalizeTags(args.tags),
    ...
    presentCall: args => ({ card: 'generic' as const, title: 'Search memory', rawInput: args.query }),
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    presentCall: args => ({ card: 'generic' as const, title: 'Forget memory', rawInput: args.id }),
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    presentCall: () => ({ card: 'generic' as const, title: 'Memory stats' }),
  }))
}

function formatHit(entry: MemoryEntry, score: number): string {
...
interface Caller {
  ...
  readonly source: { readonly sessionId: string; readonly seq: number } | undefined
}

function callerOf(exec: ToolRunContext): Caller {
  const agent = exec.agent
  if (agent === undefined) return { workspace: undefined, source: undefined }
  const session = agent.session
  if (session === undefined) return { workspace: undefined, source: undefined }

...
export function registerMemoryPrompt(
  ctx: Context,
  store: MemoryStore,
  recall: RecallCache | undefined,
  ...
  ctx.systemPrompt.section({
    ...
    text: (context) => {
      ...
      const sessionId = (context as AgentCarrier).agent?.session?.id
...
function renderRecall(query: string, hits: readonly SearchHit[]): string {
  const lines = hits.map(({ entry, score }) => {
    const meta: string[] = [entry.kind]
...
function formatInjected(entry: MemoryEntry): string {
  const meta: string[] = [entry.kind]
  if (entry.tags.length > 0) meta.push(entry.tags.join(','))
  return `- [${meta.join(' | ')}] ${entry.content}`
}

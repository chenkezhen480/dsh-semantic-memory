...
import { AutoSummarizer, collectText, extractTranscript, parseSummary } from '../summarize.ts'
...
function fakeLlm(answer: string): { stream(): AsyncIterable<unknown> } {
  return {
    async * stream() {
...
function messageEvent(type: 'user/message' | 'assistant/message', text: string): unknown {
